...
def q_learning(env, num_episodes, gamma=1.0, alpha=0.01,
    ...
    for episode in range(num_episodes):
        ...
        # s = round(env.reset(), 2)
        s = tuple([round(x, 2) for x in env.reset()])
        r_total = 0
        for step in range(max_episode_steps):
            ...
            if not done:
                s_ = tuple([round(x, 2) for x in s_])
                r_total += r
            ...
            if done or step == max_episode_steps - 1:
                ...
                if episode % 100 == 0:
                    time_stamp = time.strftime('%Y%m%d%H%M%S', time.gmtime())
                    reward_path = '/home/ur5/thesis/simple_task/scoop_grasp_2d/data/q_learning/'+time_stamp+'reward'
